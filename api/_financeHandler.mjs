...
export default async function financeHandler(request, response) {
  ...
  try {
    if (request.method === 'GET' && pathname === '/api/finance') return json(response, 200, { state: await liveStore.snapshot(role), persistence: 'ephemeral-serverless' })
    if (request.method === 'POST' && pathname === '/api/finance/cashflow-scenarios') return json(response, 201, { scenario: await liveStore.createScenario(role, await body(request), context), persistence: 'ephemeral-serverless' })
    if (request.method === 'PUT' && pathname === '/api/finance/cards') return json(response, 200, { cards: await liveStore.saveCards(role, await body(request), context), persistence: 'ephemeral-serverless' })
    const taskMatch = pathname.match(/^\/api\/finance\/tasks\/(task-[\w-]+)\/(evidence|submit|review|risk-briefing)$/)
    if (taskMatch && taskMatch[2] === 'evidence' && request.method === 'POST') { const result = await liveStore.uploadEvidence(role, taskMatch[1], await body(request), context); return result ? json(response, 201, { ...result, persistence: 'ephemeral-serverless' }) : json(response, 404, { error: 'NOT_FOUND' }) }
    if (taskMatch && taskMatch[2] === 'submit' && request.method === 'POST') { const task = await liveStore.submitTask(role, taskMatch[1], await body(request), context); return task ? json(response, 200, { task, persistence: 'ephemeral-serverless' }) : json(response, 404, { error: 'NOT_FOUND' }) }
    if (taskMatch && taskMatch[2] === 'review' && request.method === 'PATCH') { const task = await liveStore.reviewTask(role, taskMatch[1], await body(request), context); return task ? json(response, 200, { task, persistence: 'ephemeral-serverless' }) : json(response, 404, { error: 'NOT_FOUND' }) }
    if (taskMatch && taskMatch[2] === 'risk-briefing' && request.method === 'POST') { await body(request); const briefing = await liveStore.riskBriefing(role, taskMatch[1], context); return briefing ? json(response, 200, { briefing, persistence: 'ephemeral-serverless' }) : json(response, 404, { error: 'NOT_FOUND' }) }
