...
async function api(request, response, pathname, store) {
  if (request.method === 'GET' && pathname === '/api/health') {
    try {
      const state = await store.snapshot('L1_ADMIN')
      return json(response, 200, { ok: true, app: 'finans', persistence: store.persistence.kind, actors: state.team.length, modules: state.cards.length }), true
    } catch (error) { return json(response, status(error), { error: error?.message || 'SERVICE_UNAVAILABLE' }), true }
  }
  ...
  try {
    ...
    if (request.method === 'PUT' && pathname === '/api/finance/masa-layout') return json(response, 200, { layout: await store.saveMasaLayout(role, await body(request), context) }), true
...
export function createFinansServer({ dataFile = process.env.SECTRAI_FINANS_DATA_FILE || join(root, 'data', 'finance.json'), rootDirectory = root, storeOptions } = {}) { const store = new FinanceStore(dataFile, undefined, undefined, storeOptions); return createServer(async (request, response) => { const pathname = new URL(request.url || '/', 'http://localhost').pathname; if (await api(request, response, pathname, store)) return; if (pathname.startsWith('/api/')) return json(response, 404, { error: 'NOT_FOUND' }); await staticFile(request, response, pathname, rootDirectory) }) }
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) createFinansServer().listen(port, host, () => console.log(`Sectrai Finans API + static server listening on http://${host}:${port}`))
