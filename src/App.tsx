...
function App() {
  ...
  useEffect(() => { void load() }, [])
  async function addScenario() { setSaving(true); try { const scenario = await createScenario(templateId); setState((current) => ({ ...current, cashflowScenarios: [scenario, ...current.cashflowScenarios] })); setNotice('Sentetik nakit akışı senaryosu yerel JSON deposuna kaydedildi.') } catch { setNotice('Senaryo kaydedilemedi; yerel API bağlantısını kontrol edin.') } finally { setSaving(false) } }
  ...
  async function recordDecision(value: Decision) { if (!selected) return; setSaving(true); try { const updated = await decide(module, selected.id, value); setState((current) => ({ ...current, [module]: current[module].map((item) => item.id === updated.id ? updated : item) })); setNotice('Owner kararı yalnız yerel JSON deposuna kaydedildi. Sistem hiçbir dış işlem başlatmadı.') } catch { setNotice('Owner kararı kaydedilemedi; kayıt değişmedi.') } finally { setSaving(false) } }
  ...
  return <main className="app-shell">
    ...
    <section className="workspace"><header><div><p>SECTRAI FINANS / OWNER WORKSPACE</p><h1>Kanıt görünür, karar sende.</h1></div><div className="header-actions"><span className="badge">SENTETİK DEMO</span><a href="/__admin-logout">Oturumu kapat</a></div></header>
      <div className="boundary"><strong>Finansal karar destek sistemi değildir.</strong> Tüm tutarlar, müşteri adları, kanıtlar ve AI açıklamaları sentetiktir. Hiçbir karar otomatik uygulanmaz.</div>
