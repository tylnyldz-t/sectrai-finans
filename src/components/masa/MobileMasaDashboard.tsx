...
function labelFor(kind: CardKind) {
  return t(({ kpi: 'GÖSTERGE', today: 'BUGÜN', approval: 'ONAY', workspace: 'MODÜL', mega: 'TASLAK', customer: 'MÜŞTERİ', contacts: 'KİŞİLER', quote: 'TEKLİF', task: 'GÖREV', record: 'KAYIT' } as const)[kind]);
}

function lifecycle(card: DeskCard, mega: MegaStage, approval: boolean) {
  if (card.kind === 'approval') return t(approval ? 'Tamamlandı' : 'Onay bekliyor');
  if (card.kind === 'mega') return t(mega === 'creating' ? 'Çalışıyor' : 'Taslak');
  if (card.kind === 'customer') return t('Aktif');
  return t('Canlı');
}
...
export function MobileMasaDashboard({ workspace, user }: { workspace: Workspace; user: PublicUser }) {
  ...
  return (
    <div className={`mobile-masa${dark ? ' dark' : ''}`}>
      <div className="mobile-masa-banner"><span /> {t("Design Preview — yerel önizleme verisi · Arka uca yazılmaz")}</div>
      {offline && <div className="mobile-masa-offline"><span /> {t("Çevrimdışı — değişiklikler yerelde bekletilir; AI yanıt veremez")}</div>}
      <div className="mobile-masa-layout">
        ...
        <div className="mobile-masa-main">
          <header className="mobile-masa-header">
            <button className="mobile-masa-exit" onClick={goFrontDoor} title={t("KONUŞ | UYGULA ön kapısına dön")} aria-label={t("Ön kapıya (KONUŞ/UYGULA) dön")}><ArrowLeft size={16} /> {t("KONUŞ")}</button>
            <div className="mobile-masa-mark">S</div>
            <div><strong>{t(page === 'masa' ? 'Masa' : page === 'isler' ? 'İşler' : page === 'kayitlar' ? 'Kayıtlar' : 'Profil')}</strong><small>{t(workspace.title)} · <b>{t(focusId ? displayed[0]?.title ?? '' : page === 'masa' ? 'Masa (genel)' : page)}</b></small></div>
            <button className="mobile-masa-theme" onClick={() => setDark()}>{dark ? <Sun size={15} /> : <Moon size={15} />} {t(dark ? 'Açık' : 'Koyu')}</button>
          </header>
          ...
          <div className="mobile-masa-composer">
            <input value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && send()} placeholder={t(offline ? 'Çevrimdışı — mesaj bekletilecek' : "AI'ya yazın… (ör. Yeni müşteri Mega Ltd ekle)")} />
            <button title={t("Sesli giriş")} onClick={() => notify('Mikrofon isteğe bağlıdır — önizlemede metin girişini kullanın')}><Mic size={16} /></button>
            <button className="mobile-masa-send" onClick={() => send()}><Send size={16} /> {t("Gönder")}</button>
          </div>
          <nav className="mobile-masa-nav">
            ...
            <button className="mobile-masa-ai-button" title={t("AI Operatör")} onClick={() => setSheet(true)}>AI</button>
      ...
      {sheet && <><button className="mobile-masa-sheet-backdrop" aria-label={t("AI Operatör'ü kapat")} onClick={() => setSheet(false)} /><section className="mobile-masa-sheet"><button className="mobile-masa-sheet-handle" onClick={() => setSheet(false)}><span /></button><div className="mobile-masa-sheet-head"><span className="online-dot" /> <div><b>{t("AI Operatör")}</b><small>{t("Çevrimiçi · Bağlam: Masa")}</small></div><button onClick={() => setSheet(false)}><ChevronDown size={18} /></button></div><AiThread chat={chat} onAccept={acceptProposal} onReject={(id) => setChat((messages) => messages.map((item) => item.id === id ? { ...item, proposal: 'rejected' } : item))} /><div className="mobile-masa-sheet-input"><div className="mobile-masa-chips"><button onClick={() => send('Yeni müşteri Mega Ltd ekle')}>{t("Yeni müşteri Mega Ltd ekle")}</button><button onClick={() => send('Onay bekleyenleri göster')}>{t("Onay bekleyenleri göster")}</button></div><div><input value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && send()} placeholder={t("AI'ya yazın…")} /><button onClick={() => send()}><Send size={16} /></button></div></div></section></>}
      {toast && <div className="mobile-masa-toast">{t(toast)}</div>}
    </div>
  );
}

function NavButton({ active, icon, label, onClick }: { active: boolean; icon: React.ReactNode; label: string; onClick: () => void }) {
  return <button className={active ? 'active' : ''} onClick={onClick}>{icon}<span>{t(label)}</span></button>;
}

function AiThread({ chat, onAccept, onReject }: { chat: Chat[]; onAccept: (id: number) => void; onReject: (id: number) => void }) {
  return <div className="mobile-ai-thread">{chat.map((message) => <div key={message.id} className={`mobile-ai-bubble ${message.role}`}><div>{message.role === 'ai' ? t(message.text) : message.text}</div>{message.proposal === 'pending' && <div className="mobile-ai-proposal"><b>{t("Modül önerisi")}</b><span>{t("“Mega Ltd — Müşteri Taslağı” Masa’ya eklensin mi?")}</span><div><button onClick={() => onAccept(message.id)}>{t("Modülü ekle")}</button><button onClick={() => onReject(message.id)}>{t("Vazgeç")}</button></div></div>}{message.proposal === 'done' && <small className="mobile-ai-done">{t("✓ Modül Masa’ya eklendi")}</small>}</div>)}</div>;
}
...
function DeskCardView({ card, mega, megaTax, megaCity, approved, onFocus, onMove, onUpdate, onRemove, onMegaTax, onMegaCity, onFill, onApproveMega, onApprove, onOpenRecord, onAdd }: { card: DeskCard; mega: MegaStage; megaTax: string; megaCity: string; approved: boolean; onFocus: (id: string) => void; onMove: (id: string, direction: -1 | 1) => void; onUpdate: (id: string, patch: Partial<DeskCard>) => void; onRemove: (id: string) => void; onMegaTax: (value: string) => void; onMegaCity: (value: string) => void; onFill: () => void; onApproveMega: () => void; onApprove: () => void; onOpenRecord: (name: string, moduleId?: string) => void; onAdd: (kind: 'contacts' | 'quote' | 'task') => void }) {
  ...
  return <section className={`mobile-desk-card${card.pinned ? ' pinned' : ''}`}><header><button className="mobile-desk-title" onClick={() => onFocus(card.id)}><span>{labelFor(card.kind)}</span><b>{t(card.title)}</b></button><button title={t("Yukarı taşı")} onClick={() => onMove(card.id, -1)}>▲</button><button title={t("Aşağı taşı")} onClick={() => onMove(card.id, 1)}>▼</button><button title={t("Daralt")} onClick={() => onUpdate(card.id, { collapsed: !card.collapsed })}>{card.collapsed ? '+' : '–'}</button><button title={t("Sabitle")} className={card.pinned ? 'pin active' : 'pin'} onClick={() => onUpdate(card.id, { pinned: !card.pinned })}><Pin size={13} /></button><button title={t("Kapat")} onClick={() => onRemove(card.id)}><X size={14} /></button></header><div className="mobile-desk-meta"><span>{t("Kaynak:")} {t(card.kind === 'workspace' ? 'Çalışma alanı' : card.kind === 'customer' ? 'AI Operatör' : 'Sistem + AI')}</span><b>{lifecycle(card, mega, approved)}</b></div>{!card.collapsed && <div className="mobile-desk-body">{card.kind === 'kpi' && <div className="mobile-kpis">{[['Aktif modül', '—', 'çalışma alanı'], ['Kayıt', '—', 'sentetik önizleme'], ['AI önerisi', '—', 'onay bekliyor'], ['Denetim', '✓', 'kayıtlı']].map(([label, value, sub]) => <div key={label}><small>{t(label)}</small><strong>{value}</strong><span>{t(sub)}</span></div>)}</div>}{card.kind === 'today' && <div className="mobile-today">{[['—', 'Çalışma alanın hazır — AI Operatör’e ne görmek istediğini yaz'], ['—', 'Modül kartları sağdaki tuvalde; sürükleyip düzenleyebilirsin'], ['—', 'Her işlem önce önerilir, senin onayınla uygulanır']].map(([time, text], i) => <div key={i}><small>{time}</small><span>{t(text)}</span></div>)}</div>}{card.kind === 'approval' && <div className="mobile-approval"><b>{t("Derin Gıda A.Ş. — Filo sözleşmesi yenileme teklifi")}</b><span>{t("AI Operatör hazırladı · ₺184.000 / 12 ay")}</span>{approved ? <p>{t("● Onaylandı — gönderim kuyruğunda (önizleme)")}</p> : <div><button onClick={onApprove}>{t("Onayla…")}</button><button>{t("Reddet")}</button><small>{t("Onaysız gönderim yapılamaz — yetki kapısı aktif.")}</small></div>}</div>}{card.kind === 'workspace' && <div className="mobile-workspace-card"><b>{t(card.title)}</b><span>{t(card.detail ?? '')}</span><button onClick={() => onOpenRecord(card.title, card.moduleId)}>{t("Kayıtları aç →")}</button></div>}{card.kind === 'mega' && <div className="mobile-mega"><div className="mobile-mega-steps"><b>{t("Taslak")}</b> → <b className={ready ? 'current' : ''}>{t("Onay bekliyor")}</b> → <b>{t("Çalışıyor")}</b> → <b>{t("Tamamlandı")}</b></div>{mega === 'creating' ? <p className="mobile-running">{t("Kayıt doğrulanıyor…")}</p> : <><div className="mobile-mega-grid"><span>{t("Firma adı")}<b>Mega Ltd</b></span><span>{t("Tür")}<b>{t("Müşteri (Firma)")}</b></span><span>{t("Sorumlu")}<b>Tayla</b></span><span>{t("Çalışma alanı")}<b>{t("Satış")}</b></span></div><div className="mobile-missing"><b>{t("Eksik zorunlu alanlar")}</b><input value={megaTax} onChange={(event) => onMegaTax(event.target.value)} placeholder={t("Vergi numarası *")} /><input value={megaCity} onChange={(event) => onMegaCity(event.target.value)} placeholder={t("İl *")} /><button onClick={onFill}>{t("Eksikleri AI ile doldur")}</button></div><div className="mobile-mega-actions"><button disabled={!ready} onClick={onApproveMega}>{t("Onayla ve oluştur")}</button><button>{t("Vazgeç")}</button></div><small>{t("Karar insanda — AI bu düğmeye basamaz.")}</small></>}</div>}{card.kind === 'customer' && <div className="mobile-customer"><div><span>M</span><b>Mega Ltd<small>{t("Müşteri · İzmir · VN 4810 0042 66")}</small></b><em>{t("Aktif")}</em></div><p>{t("Sorumlu:")} <b>Tayla</b> {t("· Kaynak:")} <b>{t("AI Operatör")}</b></p><div className="mobile-customer-actions"><button onClick={() => onFocus(card.id)}>{t("Müşteriyi aç")}</button><button onClick={() => onAdd('contacts')}>{t("İlgili kişi ekle")}</button><button onClick={() => onAdd('quote')}>{t("Teklif oluştur")}</button><button onClick={() => onAdd('task')}>{t("Görev oluştur")}</button></div></div>}{card.kind === 'contacts' && <div className="mobile-empty-module">{t("Mega Ltd için henüz ilgili kişi yok.")}<button><Plus size={14} /> {t("Kişi ekle")}</button></div>}{card.kind === 'quote' && <div className="mobile-quote"><b>{t("Taslak · AI Operatör")}</b><span>{t("Şehirlerarası taşıma")} <strong>₺31.000</strong></span><span>{t("Depo içi elleçleme")} <strong>₺9.500</strong></span><span>{t("Toplam (aylık)")} <strong>₺46.500</strong></span><button>{t("Onaya gönder")}</button></div>}{card.kind === 'task' && <div className="mobile-empty-module"><b>{t("Mega Ltd başlangıç görüşmesi planla")}</b><span>{t("Sorumlu: Tayla · Termin: 21 Tem")}</span></div>}{card.kind === 'record' && <div className="mobile-empty-module"><b>{t(card.title)}</b><span>{t(card.detail ?? '')}</span><button>{t("AI ile devam et")}</button></div>}</div>}</section>;
}

function TasksPage({ onOpen, tasks }: { onOpen: (title: string) => void; tasks: string[][] }) {
  return <div className="mobile-list-page"><div className="mobile-list-toolbar"><input placeholder={t("İşlerde ara…")} /><div><button className="active">{t('Liste')}</button><button>{t("Pano")}</button></div></div><p>{t("Bir işe dokunmak onu Masa’ya modül olarak ekler.")}</p><div className="mobile-list">{tasks.map(([title, state, owner, due]) => <button key={title} onClick={() => onOpen(title)}><div><b>{t(title)}</b><small>{t(owner)} · {t(due)}</small>{state === 'Engellendi' && <em>{t("Engel: Depo kapasite onayı bekleniyor")}</em>}</div><span className={state.replaceAll(' ', '-').toLocaleLowerCase('tr-TR')}>{t(state)}</span></button>)}</div></div>;
}

function RecordsPage({ tab, setTab, query, setQuery, records, onOpen, tabs }: { tab: string; setTab: (value: string) => void; query: string; setQuery: (value: string) => void; records: string[][]; onOpen: (name: string, moduleId?: string) => void; tabs: string[] }) {
  return <div className="mobile-list-page"><input className="mobile-record-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("Kayıtlarda ara…")} /><div className="mobile-record-tabs">{tabs.map((item) => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{t(item)} <span>{records.filter((r) => r[0] === item).length}</span></button>)}</div><p>{t("Bir kayda dokunmak onu Masa’ya modül olarak ekler — sabitli modüller korunur.")}</p><div className="mobile-list">{records.map(([name, detail, moduleId]) => <button key={name} onClick={() => onOpen(name, moduleId)}><i>{name.charAt(0)}</i><div><b>{t(name)}</b><small>{t(detail)}</small></div><span>AI</span></button>)}</div></div>;
}

function ProfilePage({ dark, setDark, offline, setOffline, onReset, user, workspace }: { dark: boolean; setDark: () => void; offline: boolean; setOffline: (value: boolean) => void; onReset: () => void; user: PublicUser; workspace: Workspace }) {
  return <div className="mobile-profile"><section><span>{user.name.charAt(0)}</span><div><b>{user.name}</b><small>{t('Sahip · {workspace}', { workspace: workspace.title })}</small></div></section><section><label><button className={dark ? 'on' : ''} onClick={() => setDark()}><i /></button><span>{t('Koyu tema:')} <b>{t(dark ? 'Açık' : 'Kapalı')}</b></span></label><label><button><i /></button><span>{t("Sesli yanıt:")} <b>{t("Kapalı")}</b><small>{t("Varsayılan kapalı; hassas içerik otomatik seslendirilmez.")}</small></span></label><label><button className={offline ? 'on' : ''} onClick={() => setOffline(!offline)}><i /></button><span>{t("Çevrimdışı simülasyonu:")} <b>{t(offline ? 'Açık' : 'Kapalı')}</b><small>{t("Önizleme amaçlı — bağlantısız durumu gösterir.")}</small></span></label></section><section><p>{t("Modül düzeni ve Mega Ltd demo durumu bu tarayıcıda saklanır.")}</p><button className="mobile-reset" onClick={onReset}>{t("Önizlemeyi sıfırla")}</button></section></div>;
}
